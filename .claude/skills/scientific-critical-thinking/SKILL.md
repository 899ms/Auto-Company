...
Add a diagram when it clarifies a bias pathway, evidence hierarchy, or assessment workflow. A labeled Markdown table or Mermaid flowchart can express these relationships without additional software.

The separate scientific-schematics skill and its image-generation script are not bundled here. Use an image or diagram tool only if it is available in the current environment, and follow that tool's actual instructions. Otherwise, use a text diagram or omit the visual; the critical appraisal can proceed with the references below. Check labels and relationships against the analysis before presenting any diagram.
